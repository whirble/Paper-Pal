                    ...
                    <div key={message.id} className={cn('flex', {})}>
                        ...
                        <div className={cn('flex flex-col rounded-sm text-base py-2 text-dark gap-2 font-costum', {})}>
                            <div className='flex items-center justify-start gap-2'>
                                {message.role == 'user' 
                                ? (<><User className='w-5 h-5 p-4 text-white border-none rounded-full text-center bg-gradient-to-r from-gray-500 to-gray-200' /><p className='text-base font-semibold'>Vous</p></>) 
                                : (<><AudioLines className='w-5 h-4 text-white p-4 border-none rounded-full text-center bg-gradient-to-r from-gray-500 to-gray-200' /><p className='text-base font-semibold'>PaperPal</p></>) 
                                }
                            </div>
